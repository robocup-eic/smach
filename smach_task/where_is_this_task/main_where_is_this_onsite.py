...
def listen_for_intent(stt):
    while True:
        ...
        if time.time()-start_time > 8:
            speak("Please repeat the question again after the sound")
            stt.listen()
            start_time = time.time()
...
class Check_location(smach.State):
    ...
    def execute(self, userdata):
        ...
        def left_or_right (robot_pose= Pose(), furniture_name= str()):
            ...
            yaw = math.radians(euler_from_quaternion (orientation_list)[2])
            if -45 < yaw <= 45:
                # <-
                robox = robot_position.x
                if robox > fur_position.x:
...
class Lost(smach.State):
    def __init__(self):
        rospy.loginfo('Initiating Lost state')
        smach.State.__init__(self, outcomes = ['continue_FOLLOW','continue_Navigate_information_point'])
        self.test == True
    def execute(self, userdata):
        rospy.loginfo('Executing Lost state')
        # wait for "follow me" command
        speak("if you want me to follow you")
        speak("Please say Hey walkie follow me")
        if self.test:
            return 'continnue_FOLLOW'
        # if it takes too long the robot will go back to information point
        else:
            return 'continue_Navigate_information_point'
